...
b=fviz_pca_biplot(res.pca, label = "var", habillage=iris$Species,
                addEllipses=TRUE, ellipse.level=0.95,
                ggtheme = theme_bw())
set.seed(5753)
data <- data.frame(x = rnorm(1000))
set.seed(19191)   
data2 <- data.frame(x = c(rnorm(500), rnorm(500, 3, 2)),
                    group = as.factor(c(rep(1, 500), rep(2, 500))))
c=ggplot(data2, aes(x = x, fill = group)) +            
  geom_histogram(alpha = 0.5, position = "identity")+theme_bw()
set.seed(070510)
d <- data.frame(x = rnorm(2000))
d=ggplot(d, aes(x, fill = cut(x, 100))) +
  geom_histogram(show.legend = FALSE)+theme_bw()
library(gridExtra)
